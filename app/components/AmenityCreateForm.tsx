...
export default function AmenityCreateForm({ onSuccess }: AmenityCreateFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: "success", text: `"${amenityNameToDisplay(newAmenity.name)}" created successfully!` });
      setFormData({ name: "" });
      onSuccess?.();
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error("Error creating amenity:", error);
      setMessage({ type: "error", text: `Failed to create amenity: ${errorMessage}` });
    } finally {
      setIsSubmitting(false);
    }
  };
